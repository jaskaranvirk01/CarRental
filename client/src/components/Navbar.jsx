...
const Navbar = () => {
    ...
    return (
        <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.5 }}
            ...
            <Link to="/">
                <motion.img whileHover={{ scale: 1.05 }} src={assets.logo} alt="Logo" className="h-8 hover:scale-105 transition-all duration-300" />
            </Link>
            ...
            <div className={`max-sm:fixed max-sm:h-screen max-sm:w-full max-sm:top-16 max-sm:border-t border-borderColor right-0 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-8 max-sm:p-4 transition-all duration-300 z-50 ${location.pathname === "/" ? "bg-light" : "bg-white"} 
                ...
                <div className="flex max-sm:flex-col items-start sm:items-center gap-6">
                    ...
                    <button className="cursor-pointer px-8 py-2 bg-primary hover:bg-primary-dull transition-all text-white rounded-lg " onClick={() => { user ? logout() : setShowLogin(true) }}>
